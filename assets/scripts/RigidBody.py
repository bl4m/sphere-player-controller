...
from panda3d.bullet import BulletRigidBodyNode, BulletPlaneShape, BulletBoxShape, BulletSphereShape, BulletCapsuleShape, XUp, YUp, ZUp, BulletTriangleMesh, BulletTriangleMeshShape, BulletDebugNode
from panda3d.bullet import BulletWorld as pandaBulletWorld
...
class RigidBody(PhysicsBody):
    ...
    @property
    def mass(self):
        return self.rigid_body_node.getMass()
    @mass.setter
    def mass(self,mass):
        self.rigid_body_node.setMass(mass)
        self.attach()
...
class BulletWorld(pandaBulletWorld):
    def SetGravity(self,vector:Vec3):
        """
        sets the acceleration due to gravity
        """
        self.setGravity(vector)
    def DoPhysics(self,dt:float):
        """
        update the bullet world to do physics
        """
        self.doPhysics(dt)
